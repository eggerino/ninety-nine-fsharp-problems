...
module Lists =
    ...
    let rec flatten =
        function
        | [] -> []
        | flattenDef.One x :: rest -> x :: flatten rest
        | flattenDef.Many xs :: rest -> flatten xs @ flatten rest
    ...
    let encode2 list =
        let toItem cnt item =
            match cnt with
            | 1 -> encodeDef.One item
            | _ -> encodeDef.Many(cnt, item)
    ...
    let decode list =
        ...
        let rec aux acc =
            function
            | [] -> acc
            | encodeDef.One x :: rest -> aux (x :: acc) rest
            | encodeDef.Many(cnt, item) :: rest -> aux (pushMany acc (cnt, item)) rest
